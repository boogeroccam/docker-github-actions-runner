FROM debian:12
LABEL maintainer="ilia@occam.fi"
...
# install sops
RUN curl -L https://github.com/getsops/sops/releases/download/${SOPS_VER}/sops-${SOPS_VER}.linux.amd64 -o /usr/local/bin/sops &&\
    chmod +x /usr/local/bin/sops
# install age
RUN curl -Ls https://github.com/FiloSottile/age/releases/download/v1.2.0/age-v1.2.0-linux-amd64.tar.gz | tar xz --strip-components=1 -C /usr/local/bin age/age
...
# install helm
RUN curl https://get.helm.sh/helm-canary-linux-amd64.tar.gz | tar xz --strip-components=1 -C /usr/local/bin linux-amd64/helm
...
RUN export UBUNTU_CODENAME=jammy && \
    ...
    sudo apt update && sudo apt install -y --no-install-recommends ansible
...
COPY prepare.sh token.sh entrypoint.sh app_token.sh /
RUN chmod +x /token.sh /entrypoint.sh /app_token.sh /prepare.sh
